...
const EXIT_INTENT_PATTERNS = [
  'n├úo quero', 'nao quero', 'n├úo desejo', 'nao desejo',
...
const repairMojibake = (text: string): string => {
  ...
  try {
    ...
    const hasPortugueseAccents = /[├í├á├ú├ó├®├¬├¡├│├┤├Á├║├╝├º├ü├Ç├â├é├ë├è├ì├ô├ö├ò├Ü├£├ç]/.test(candidate);
...
const resolveAgentRouteDecision = async (
  ...
  if (
    ...
    return {
      ...
      reason: 'Prompt comercial do agente e intencao inicial compat├¡vel com venda/consulta comercial.',
    ...
    switch (nodeType) {
      ...
      case 'option': {
        if (getProperty(session.flow_variables, '_invalidOption') === true) {
          await sendOmniChannelMessage(session, currentWorkspace, "Op├º├úo inv├ílida. Por favor, tente novamente.");
      ...
      case 'condition': {
        ...
        switch (op) {
          ...
          default:
            console.warn(`[Flow Engine] Operador de condi├º├úo desconhecido: "${currentNode.conditionOperator}" (normalizado: "${op}")`);
            conditionMet = false;
        }
      ...
      case 'time-of-day': {
        ...
        try {
          ...
          if (startTimeStr && endTimeStr && /^\d{2}:\d{2}(?::\d{2})?$/.test(startTimeStr) && /^\d{2}:\d{2}(?::\d{2})?$/.test(endTimeStr)) {
            ...
            console.warn(`[Flow Engine - ${session.session_id}] time-of-day: hor├írios inv├ílidos ou ausentes (start="${startTimeStr}" end="${endTimeStr}"). Considerando fora do intervalo.`);
            isInTimeRange = false;
          }
      ...
      case 'code-execution': {
        ...
        if (currentNode.codeSnippet && varName) {
          try {
            ...
            console.log(`[Flow Engine Code] Vari├ível "${varName}" definida com sucesso.`);
          } catch (e: any) {
            console.error(`[Flow Engine - ${session.session_id}] Erro ao executar c├│digo no sandbox:`, e);
            setProperty(session.flow_variables, varName, { error: e.message });
          }
        } else {
          console.warn(`[Flow Engine] N├│ 'Executar C├│digo' sem script ou vari├ível de sa├¡da definida.`);
        }
        nextNodeId = findNextNodeId(currentNode.id, 'default', connections);
        break;
      }
