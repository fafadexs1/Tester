...
// Helper to find the correct workspace, now also checking owner
async function findWorkspace(name: string, owner?: string): Promise<WorkspaceData | null> {
    const workspace = await loadWorkspaceByNameFromDB(name);
    if (!workspace) return null;
    // If an owner is specified for the lookup, ensure it matches.
    if (owner && workspace.owner !== owner) {
        console.warn(`[API Evolution Webhook] Workspace found by name "${name}", but owner mismatch. Expected: ${owner}, Found: ${workspace.owner}`);
        return null; 
    }
    return workspace;
}
...
export async function POST(request: NextRequest, context: { params: { username: string; workspaceName: string } }) {
  const { username, workspaceName } = context.params;
  ...
  console.log(`[API Evolution Webhook] POST received for user: "${decodedUsername}", workspace: "${decodedWorkspaceName}"`);
  ...
  try {
    ...
    loggedEntry = await storeRequestDetails(request, parsedBody, rawBody, `${decodedUsername}/${decodedWorkspaceName}`);
    ...
    if (eventType !== 'messages.upsert' || !senderJid || !receivedMessageText || !instanceName || !evolutionApiBaseUrl) {
      ...
      console.log(`[API Evolution Webhook] Logged, but no flow execution triggered for "${decodedUsername}/${decodedWorkspaceName}". Reason: ${reason}.`);
      return NextResponse.json({ message: `Webhook logged, but no flow execution: ${reason}.` }, { status: 200 });
    }
    
    const workspace = await findWorkspace(decodedWorkspaceName, decodedUsername);

    if (!workspace || !workspace.nodes || workspace.nodes.length === 0) {
      console.error(`[API Evolution Webhook] Workspace "${decodedWorkspaceName}" for user "${decodedUsername}" not found or empty.`);
      await sendWhatsAppMessageAction({baseUrl: evolutionApiBaseUrl, apiKey: evolutionApiKey || undefined, instanceName, recipientPhoneNumber: senderJid.split('@')[0], messageType:'text', textContent: `Desculpe, o fluxo de trabalho "${decodedWorkspaceName}" não foi encontrado ou está vazio.`});
    ...
    if (session) {
        console.log(`[API Evolution Webhook - ${sessionId}] Existing session found. Node: ${session.current_node_id}, Awaiting: ${session.awaiting_input_type}`);
      if (session.current_node_id === null && session.awaiting_input_type === null) {
          console.log(`[API Evolution Webhook - ${sessionId}] Session is in a paused (dead-end) state.`);
          session = null;
      } else {
        ...
        if (session.awaiting_input_type && session.awaiting_input_details) {
            ...
            if (awaitingNode) {
                ...
                console.warn(`[API Evolution Webhook - ${sessionId}] Awaiting node ${originalNodeId} not found, restarting flow.`);
                session = null;
            }
        } else {
           session = null;
    ...
    if (!session) {
      console.log(`[API Evolution Webhook - ${sessionId}] No active session or session was reset. Looking for a trigger to start a new one.`);
      ...
      if (!triggerNameToUse) {
         console.log(`[API Evolution Webhook - ${sessionId}] No matching keyword trigger and no enabled webhook trigger found. Flow will not start.`);
      ...
      if(!initialNodeId){
          console.error(`[API Evolution Webhook] Start node trigger '${triggerNameToUse}' is not connected.`);
    ...
    console.error(`[API Evolution Webhook - POST ERROR] Error for "${decodedUsername}/${decodedWorkspaceName}":`, error.message, error.stack);
    return NextResponse.json({ error: "Internal server error processing webhook.", details: error.message }, { status: 500 });
  }
}

export async function GET(request: NextRequest, context: { params: { username: string, workspaceName: string } }) {
  const { username, workspaceName } = context.params;
  const decodedWorkspaceName = decodeURIComponent(workspaceName.replace(/_/g, ' '));
  const decodedUsername = decodeURIComponent(username.replace(/_/g, ' '));
  try {
    ...
    if (workspace) {
      return NextResponse.json({
        message: `Workspace "${decodedWorkspaceName}" for user "${decodedUsername}" found. Ready to receive POST webhooks.`,
        workspaceId: workspace.id,
      }, { status: 200 });
    } else {
      return NextResponse.json({ error: `Workspace "${decodedWorkspaceName}" for user "${decodedUsername}" not found.` }, { status: 404 });
    }
  } catch (error: any) {
...
export async function PUT(request: NextRequest, context: { params: { username: string, workspaceName: string } }) {
  return POST(request, context);
}
export async function PATCH(request: NextRequest, context: { params: { username: string, workspaceName: string } }) {
  return POST(request, context);
}
export async function DELETE(request: NextRequest, context: { params: { username: string, workspaceName: string } }) {
  return POST(request, context);
}
