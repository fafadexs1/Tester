...
export async function POST(request: NextRequest, { params }: { params: { workspaceName: string } }) {
  ...
  try {
    ...
    if (!rawBody || rawBody.trim() === '') {
      parsedBody = { message: "Request body was empty." };
    } else {
      try {
        parsedBody = JSON.parse(rawBody);
      } catch (jsonError: any) {
        parsedBody = { raw_text: rawBody, parse_error: jsonError.message };
      }
    }
  ...
  try {
    ...
    if (workspace) {
      return NextResponse.json({
        message: `Workspace "${decodedWorkspaceName}" found. Ready to receive POST webhooks.`,
