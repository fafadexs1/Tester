...
const parseCurlCommand = (command: string): ParsedCurlResult => {
...
const NodeCard: React.FC<NodeCardProps> = React.memo(({
  ...
  const handleApplyCurlImport = useCallback(() => {
    try {
      ...
      const sections: string[] = ['URL', 'metodo'];
      if (mappedHeaders.length) sections.push('headers');
      if (mappedQuery.length) sections.push('query string');
      if (parsed.bodyType === 'form-data') {
        sections.push('corpo form-data');
      } else if (parsed.bodyType === 'json') {
        sections.push('corpo JSON');
      } else if (parsed.bodyType === 'raw') {
        sections.push('corpo raw');
      } else {
        sections.push('corpo vazio');
      }

      const authType = parsed.auth?.type ?? 'none';
      updates.apiAuthType = authType;
      if (authType === 'bearer') {
        updates.apiAuthBearerToken = parsed.auth?.bearerToken || '';
        updates.apiAuthBasicUser = '';
        updates.apiAuthBasicPassword = '';
        sections.push('autenticacao');
      } else if (authType === 'basic') {
        updates.apiAuthBasicUser = parsed.auth?.basicUser || '';
        updates.apiAuthBasicPassword = parsed.auth?.basicPassword || '';
        updates.apiAuthBearerToken = '';
        sections.push('autenticacao');
      } else {
        updates.apiAuthBearerToken = '';
        updates.apiAuthBasicUser = '';
      ...
      toast({
        title: 'Comando cURL importado!',
        description: `Preenchemos ${sections.join(', ')}.`,
      });
      ...
      setCurlImportError(error?.message || 'Nao foi possivel interpretar este comando cURL.');
    }
  ...
  const getApiPreviewResult = (
    expression: string | undefined,
    key: string,
    options?: { extractAs?: 'single' | 'list'; itemField?: string }
  ): PreviewResult => {
    if (!expression || expression.trim() === '') {
      return { type: 'empty', message: 'Defina a expressão JSONata para visualizar o resultado.' };
    }
    ...
    const rawResult = apiPreviewResults[key];
    if (!rawResult || rawResult.type !== 'success') {
      return rawResult || { type: 'pending', message: 'Calculando pré-visualização...' };
    }

    if (options?.extractAs !== 'list' || !options.itemField) {
      return rawResult;
    }
    ...
    const plucked = list
      .map(item => {
        ...
        if (typeof item === 'object') {
          return getProperty(item, options.itemField!);
        }
        return item;
      })
      .filter(item => item !== undefined && item !== null);

    return { type: 'success', value: plucked };
  };
...
minhaFuncao(variables);`}
  ...
  return (
    <>
      ...
      {/* API CURL Import Dialog */}
      {node.type === 'api-call' && (
        <Dialog
          open={isCurlImportDialogOpen}
          onOpenChange={(open) => {
            setIsCurlImportDialogOpen(open);
            if (!open) {
              setCurlImportError(null);
            }
          }}
        >
          <DialogContent className="sm:max-w-2xl" data-no-drag="true">
            <DialogHeader>
              <DialogTitle>Importar comando cURL</DialogTitle>
              <DialogDescription>Cole o comando completo para preencher URL, metodo, headers, query e corpo automaticamente.</DialogDescription>
            </DialogHeader>
            <div className="space-y-3 py-2">
              <Textarea
                ...
                rows={8}
                className="font-mono text-xs"
                placeholder={`curl --location -g 'https://api.exemplo.com/endpoint' \
  --header 'Authorization: Bearer {{token}}' \
  --data '{\"contrato\": \"308\"}'`}
              />
              {curlImportError ? (
                <p className="text-sm text-destructive">{curlImportError}</p>
              ) : (
                <p className="text-xs text-muted-foreground">Campos detectados podem ser ajustados depois da importacao.</p>
              )}
            </div>
            <DialogFooter>
              <DialogClose asChild>
              ...
              <Button onClick={handleApplyCurlImport} disabled={!curlInput.trim()}>
                Importar
              </Button>
            </DialogFooter>
          </DialogContent>
